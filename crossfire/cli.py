"""CLI for ``crossfire run`` and ``crossfire clean``."""
...
from crossfire.core.domain import CrossfireConfiguration, Mode, RunParameters, Task
...
def run(
    ...
    try:
        result = asyncio.run(_execute(configuration, parameters, archive_path, verbose=verbose))
...
async def _execute(
    ...
    *,
    ...
    try:
        ...
        if tui:
            tui.finish(orchestrator.cost_tracker.summarize())
        return result
    except Exception:
        if tui:
            tui.report_error()
